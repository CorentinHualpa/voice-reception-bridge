...
export function createPizzeria(opts = {}) {
  ...
  function run(name, args, ctx = {}) {
    if (name === "chiffrer_commande") {
      ...
      return { lignes: c.lignes, nb_pizzas: c.nbPizzas, total_eur: c.total, total_oral: frEuros(c.total), ...(disparus.length ? { attention: `Absents de ce chiffrage alors qu'ils étaient dans le précédent : ${disparus.join(", ")}. Chaque chiffrage doit reprendre la commande ENTIÈRE. Si le client ne les a pas annulés, rechiffrer avec eux avant d'annoncer le total.` } : {}), ...(c.inconnus.length ? { inconnus: c.inconnus, consigne: "Ces articles ne sont pas sur la carte : proposer les plus proches ou faire répéter." } : {}), ...(c.prixManquants.length ? { prix_inconnus: c.prixManquants, consigne_prix: "Ne pas inventer ces prix : dire qu'ils seront confirmés au comptoir." } : {}) };
    }
  ...
  return { tools, run, contexteAppel, carteTexte, consigneCloture, commandes: () => commandes, messages: () => messages };
}
