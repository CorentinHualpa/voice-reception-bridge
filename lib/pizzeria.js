...
// Garde de cloture, commune aux deux modes (outils locaux ou outils de Dale Voz) : le modele peut
// remercier « pour votre commande » sans avoir appele enregistrer_commande (banc du 15/09/2026).
// Seule la phrase de cloture definitive declenche : « c'est note » se dit aussi en cours de commande.
...
export function consigneClotureCommande(texte, { outils = [], dejaEnregistree = false } = {}) {
  if (!CLOTURE_COMMANDE.test(texte || "")) return null;
  if (outils.includes("enregistrer_commande") || dejaEnregistree) return null;
...
// Les outils de commande, quel que soit celui qui les execute.
export const OUTILS_DE_COMMANDE = new Set(["chiffrer_commande", "proposer_heure_retrait", "enregistrer_commande", "transmettre_message"]);
...
export function createPizzeria(opts = {}) {
  ...
  function consigneCloture(texte, ctx = {}) {
    return consigneClotureCommande(texte, {
      outils: ctx.outils,
      dejaEnregistree: Boolean(ctx.callSid && commandes.some((c) => c.callSid === ctx.callSid)),
    });
  }
  ...
  function contexteAppel() {
    ...
    // Appel de controle du 17/09/2026 : sur « il vous reste de la Salmon Joe ? », l'agent disait bien non,
    // puis cherchait AUSSI dans la base de connaissance, tombait sur « la pizza du moment » et l'annoncait
    // a tort comme telle. La disponibilite du jour n'est que dans ce contexte.
    if (etat?.poste) {
      if (!epuises.length && !limites.length) lignes.push("Aujourd'hui, toute la carte est disponible.");
      lignes.push("La disponibilité du jour est entièrement donnée ici : pour une question « il vous reste… ? », répondre à partir de ces lignes et de la carte, sans jamais chercher dans la base de connaissance.");
    }
    return lignes.join("\n");
  }
