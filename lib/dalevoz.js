...
async function appeler(chemin, { methode = "GET", tenantId = null, corps = null, timeout = TIMEOUT_MS } = {}) {
  ...
  try {
    ...
    if (!r.ok) {
      console.error(`[dalevoz] ${methode} ${chemin} -> ${r.status} ${texte.slice(0, 200)}`);
      return null;
    }
    return json;
  } catch (e) {
...
/** À quel espace et quel agent appartient le numéro appelé. null = numéro inconnu. */
export function resoudreNumero(numero) {
  if (!dalevozActif || !numero) return Promise.resolve(null);
  return appeler(`/api/voice/telephone?numero=${encodeURIComponent(numero)}`);
}
