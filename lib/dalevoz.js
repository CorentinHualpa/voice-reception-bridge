...
export function executerOutil({ tenantId, agentSlug, outil, args, sessionId, locale, appel }) {
  if (!dalevozActif) return Promise.resolve(null);
  return appeler("/api/voice/tool", {
    methode: "POST",
    tenantId,
    // `appel` (CallSid, numero appelant, recapitulatif confirme) : la prise de commande de Dale Voz
    // en a besoin, une commande par appel. La plateforme ne le croit que venant du pont.
    corps: { agent: agentSlug, tool: outil, args: args || {}, ...(sessionId ? { sessionId } : {}), ...(locale ? { locale } : {}), ...(appel ? { appel } : {}) },
