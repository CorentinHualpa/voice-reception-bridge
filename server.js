...
// 17/09/2026 : descendu de 400 a 250 ms, comme sur Palazzo. Le son n'est toujours lache qu'a la fin du tour, donc
// la fin de phrase n'est pas raccourcie et un nom epele garde sa tolerance ; on gagne 150 ms sur TOUS les tours.
...
// `entreeNouvelle`), au-dela duquel la reponse est jetee comme une redite.
// ⛔ COUPEE PAR DEFAUT SUR DANY (17/09/2026), alors qu'elle tourne en production sur Palazzo. Au banc d'appel
// local de Motralec elle jette des reponses parfaitement legitimes, plusieurs par appel et jusqu'a des tours
// entiers du parcours de devis (« Je note votre besoin… », « Quelle est votre adresse mail… »). Les deux ponts
// ne voient pas les transcriptions au meme rythme : Dany est en demi-duplex, valide ses tours a 1000 ms et fait
// epeler des noms et des emails, la ou Palazzo est en duplex a 900 ms. Ne la rallumer qu'apres avoir mesure,
// sur de vrais appels de Dany, quand sa transcription arrive par rapport au premier son.
const REDITE_ATTENTE_MS = Number(process.env.REDITE_ATTENTE_MS ?? 0);
...
wss.on("connection", (twilio) => {
  ...
  // `entreeNouvelle` dit qu'il y a bien de quoi repondre : une transcription du client non vide et pas encore
  // utilisee, un resultat d'outil, ou une consigne poussee par le pont.
  let entreeNouvelle = true;                // l'accueil n'a pas d'entree et ne doit pas etre bloque
  // ⚠ Un « mmm » dit juste apres une phrase s'AJOUTE au meme message chez Grok (meme `item_id`) : son texte a
  // l'air neuf alors que le message a deja servi. La garde suit donc l'IDENTIFIANT, pas le contenu.
  let dernierItemClient = "", itemClientConsomme = "";
  let dernierTexteClient = "", texteClientConsomme = "";
  ...
  // a ete entendu) et on lui fera dire le texte de la doublure a la fin du tour, pour qu'elle ne le redise pas.
  function doublurePrendLaMain() {
    ...
    // sert aussi de garde a la coupure de parole (`reponseCoupee !== respSeq`), et le client ne pourrait alors
    // plus couper une reponse doublee (sans objet sur Dany, qui est en demi-duplex, mais la regle reste).
    ...
    // repond a la question suivante SANS avoir la reponse precedente et redit tout.
    marquerGeneration();
    console.log(`[doublure] prend la main sur la reponse n°${respSeq}, ${Date.now() - debutReponseMs} ms apres sa creation ${t()} sid=${callSid}`);
    return true;
  }
  function ouvrirDoublure({ modele, vitesse, effort, sessionInstructions }) {
      ...
      sur: {
        ...
        finie: (statut, tourDoublure) => {
          ...
          if (texte) {
            ...
            if (reponseCoupee !== tourDoublure.marque && /remercie pour votre appel/i.test(texte)) closingSaid = true;
          }
          console.log(`[doublure] reponse n°${tourDoublure.marque} finie (${statut}), ${texte.length} car${texte ? ` : « ${texte.slice(0, 120)} »` : ""} ${t()} sid=${callSid}`);
  ...
  function noterEntreeClient(itemId, texte) {
    // Repli sur le numero de tour quand xAI n'envoie pas d'`item_id` : sans lui l'identifiant resterait vide,
    // aucune entree ne serait jamais reconnue comme neuve, et TOUTES les reponses passeraient pour des redites
    // (constate au banc sur l'agent de Dany : cinq reponses jetees sur huit, dont celles de la doublure).
    const id = itemId || `tour-${toursValides}`;
    dernierItemClient = id;
    const t = String(texte || "").trim();
    if (!t) return;
    // Entree neuve si le message est un autre, OU si son texte n'est pas un simple prolongement de celui qui a
    // deja servi. ⚠ Les deux conditions sont necessaires : l'identifiant seul rate les tours que Grok range
    // dans le MEME message (il ne clot la prise de parole qu'a son rythme, et plusieurs tours du pont peuvent y
    // tomber), et le texte seul rouvre le piege du « mmm », qui s'ajoute au message precedent et le fait
    // paraitre neuf alors qu'il a deja ete repondu.
    if (id !== itemClientConsomme || !normLine(t).startsWith(normLine(texteClientConsomme))) entreeNouvelle = true;
    dernierTexteClient = t;
  }
  function consommerEntreeClient() {
    itemClientConsomme = dernierItemClient;
    texteClientConsomme = dernierTexteClient;
  ...
  function terminerReponse(e) {
    ...
    rediteTranchee = true;
    // Ce qui vient d'etre dit est consomme : la prochaine reponse aura besoin d'une entree a elle. Consommee
    // meme si le client a coupe, il a coupe pour dire autre chose. ⚠ Ne PAS remettre ce drapeau a chaque
    // validation de tour : une anticipation annulee revalide le meme tour, dont la transcription est deja
    // arrivee et ne reviendra pas, et on jetterait une reponse parfaitement legitime.
  ...
  async function openGrok() {
    ...
    grok.on("open", () => {
      ...
      // son historique lui arrive en texte.
      if (TOURS_PAR_LE_PONT && HEDGE_APRES_MS > 0) ouvrirDoublure({ modele: GROK_MODEL, vitesse: GROK_SPEED, effort: GROK_REASONING, sessionInstructions });
    });
